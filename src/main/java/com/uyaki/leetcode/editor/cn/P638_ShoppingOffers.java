...
// 0 <= special[i][j] <= 50 
// 
// Related Topics 位运算 记忆化搜索 数组 动态规划 回溯 状态压缩 👍 278 👎 0
...
public class P638_ShoppingOffers {
    ...
    class Solution {
        ...
        public int shoppingOffers(List<Integer> price, List<List<Integer>> special, List<Integer> needs) {
            ...
            int n = price.size();
            // 过滤不会造成优惠的礼包
            List<List<Integer>> niceSpecial = new ArrayList<>();
            for (List<Integer> sp : special) {
                ...
                // 如果大礼包是空的，可以跳过，如果商品总价 > 礼包价格，说明是有优惠的
                if (totalCount > 0 && totalPrice > sp.get(n)) {
                    niceSpecial.add(sp);
                }
            }
            return dfs(price, special, needs, niceSpecial, n);
        }

        private int dfs(List<Integer> price, List<List<Integer>> special, List<Integer> curNeeds, List<List<Integer>> niceSpecial, int n) {
            //动态方程
            // dp[neends] = min [price - dp[neends - neends(i)]]
            if (!memo.containsKey(curNeeds)) {
                ...
                for (List<Integer> curSpecial : niceSpecial) {
                    // 大礼包价格
                    ...
                    for (int i = 0; i < n; i++) {
                        ...
                        //计算购买大礼包 cur_special 后新的购物清单 nxt_needs , 可以买就加入
                        nxtNeeds.add(curNeeds.get(i) - curSpecial.get(i));
                    }
                    // 只要nxtNeeds.size() == n,说明，大礼包里的所有商品都能购买
                    if (nxtNeeds.size() == n) {
                        // 递归地计算满足购物清单 nxt_needs 所需花费的最低价格 nxt_price；
                        ...
                        minPrice = Math.min(minPrice, dfs(price, special, nxtNeeds, niceSpecial, n) + specialPrice);
                    }
                }
