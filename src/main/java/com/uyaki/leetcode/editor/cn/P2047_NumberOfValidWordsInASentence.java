 ...
 * 句子中的有效单词数
 */
public class P2047_NumberOfValidWordsInASentence {
    public static void main(String[] args) {
        //测试代码
        Solution solution = new P2047_NumberOfValidWordsInASentence().new Solution();
    }
    ...
    class Solution {
        public int countValidWords(String sentence) {
            ...
            int ans = 0;
            for (String word : arr) {
                // word.length() > 0 防止空格开头，切割出空字符串
                if (word.length() > 0 && checkWorld(word)) {
                    ans++;
                }
            }
            return ans;
