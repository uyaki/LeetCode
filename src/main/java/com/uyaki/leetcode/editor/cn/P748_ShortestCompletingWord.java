...
public class P748_ShortestCompletingWord {
    public static void main(String[] args) {
        //测试代码
        Solution solution = new P748_ShortestCompletingWord().new Solution();
    }
    ...
    class Solution {
        public String shortestCompletingWord(String licensePlate, String[] words) {
            ...
            for (String word : words) {
                if (word.length() >= map.size() && word.length() < minLength && isCompletingWord(word, map)) {
        ...
        private boolean isCompletingWord(String word, HashMap<Character, Integer> map) {
            HashMap<Character, Integer> wordMap = new HashMap<>();
            for (char ch : word.toCharArray()) {
                wordMap.put(ch, wordMap.getOrDefault(ch, 0) + 1);
            }
            for (Character key : map.keySet()) {
                if (wordMap.getOrDefault(key, 0) < map.get(key)) {
                    return false;
                }
            }
            return true;
